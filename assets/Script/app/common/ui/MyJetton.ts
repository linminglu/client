...
@ccclass
export default class MyJetton extends cc.Component {
    ...
    initView() {
        ...
        if (gameState == 1) {  //开奖阶段
            ...
            if ((enterGameMsg.openTotalTime - this.gameJettonTime) < enterGameMsg.remainTime) {
                let money = GameManager.instance.getSucAreaTotalsList(this.betAreaIdx).money
    ...
    gameEnterBackGround(eventName: string, isBackGround: boolean = true) {
        ...
        if (!isBackGround) {
            FuncUtil.delayFunc(function () {
                let money = GameManager.instance.getSucAreaTotalsList(self.betAreaIdx).money
    ...
    updateView(jettonnNum: number = 0) {
        if (!cc.isValid(this.jetton)) {
            ResCfg.loadPrefab(this, "jettonView", function (self, prefab) {
                ...
                self.jetton.getComponent('JettonView').initView(jettonnNum, true)
            }, false, true)
        } else {
            this.jetton.getComponent('JettonView').initView(jettonnNum, true)
        }
    }
