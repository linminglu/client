...
export default class ScrollView1 extends cc.Component {
    ...
    onLoad() {
        ...
        ResCfg.loadPrefab(this, "gameDataItem", function (self, prefab) {
            for (let i = 0; i < 6; i++) {
                self.recordArr[i] = cc.instantiate(prefab);
                self.Content.addChild(self.recordArr[i]);
            }
            RecordController.instance._sendAgreementFun(1, null)
        })
        ...
        this.scroll.node.on("bounce-bottom", function () {
            ...
            if (self.countNum + 1 <= self.pageCount && (self.CoordCurrent.y > self.CoordMax.y + 33.5)) {
                ...
                RecordModel.instance.sendParamFun(1, self.countNum)
            }
    ...
    onEnable() {
        RecordModel.instance.registerModelChanged("COMMON_RECORD_DATA_B", this.upRecordDataListFun1, this)
    }


    onDisable() {

        RecordModel.instance.unregisterModelChanged("COMMON_RECORD_DATA_B", this.upRecordDataListFun1, this)
    ...
    upRecordDataListFun1(eventName: string, data) {   //列表刷新

        let dataArr = data
        let dataLen = dataArr.length
        let recordArrLen = this.recordArr.length
        for (let i = 0; i < 6; i++) {
            this.recordArr[i].getComponent("GameDataItem")._initLblFun();
        }
        for (let i = 0; i < dataArr.length; i++) {
            if (i < recordArrLen) {
                this.recordArr[i].getComponent("GameDataItem").recordGameDataFun(dataArr[i], this.GameTyp);
            } else {
                let prefab = NodePoolMgr.instance.getNood(NodePoolKey.GAME_HISTORY)
                if (cc.isValid(prefab)) {
                    this.Content.addChild(prefab)
                    prefab.getComponent("GameDataItem").recordGameDataFun(dataArr[i], this.GameTyp);
                    ...
                    ResCfg.loadPrefab(this, "gameDataItem", function (self, Prefab) {
                        ...
                        cView.getComponent("GameDataItem").recordGameDataFun(dataArr[i], self.GameTyp);
                    })
                }
            }
        }
        if (dataLen > 6) {
            ...
            for (let i =recordArrLen -1 ; i >= dataLen ; i--) {
                ...
                if (cc.isValid(obj1)) {
                    NodePoolMgr.instance.putNood(obj1,NodePoolKey.GAME_HISTORY)
                }
            }
        } else {
            let obj2 = null
            for (let i = recordArrLen -1; i >= 6; i--) {
                ...
                if (cc.isValid(obj2)) {
                    NodePoolMgr.instance.putNood(obj2,NodePoolKey.GAME_HISTORY)
                }
